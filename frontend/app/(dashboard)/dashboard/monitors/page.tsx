...
import { useGetMonitorsQuery, useUpdateMonitorStatusMutation, useDeleteMonitorMutation } from "@/store/api/monitorApi";
...
const demoMonitors: DemoMonitor[] = [
    ...
    },
    {
        ...
        stats: {
            ...
            lastIncident: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(),
        },
    },
    {
        id: 4,
...
export default function MonitorsListPage() {
    ...
    const getStatusBadge = (status: string | null) => {
        ...
        const statusConfig = MONITOR_STATUS[status as keyof typeof MONITOR_STATUS];
    ...
    const handleToggleStatus = async (monitorId: number, currentStatus: boolean) => {
        try {
            ...
            toast({
                ...
                description: `Monitor ${currentStatus ? "paused" : "resumed"} successfully`,
            });
        } catch (error) {
            toast({
                title: "Error",
    ...
    const handleDeleteMonitor = async (monitorId: number, monitorUrl: string) => {
        if (!confirm(`Are you sure you want to delete the monitor for ${monitorUrl}?`)) {
            return;
        }
                        ...
                        <Card>
                            ...
                            <CardContent>
                                {isLoading ? (
                                    ...
                                    <div className="text-center py-8">
                                        <p className="text-red-600">Error loading monitors</p>
                                        <p className="text-gray-600 text-sm mt-1">Using demo data</p>
                                    </div>
                                                ...
                                                <TableRow key={monitor.id}>
                                                    ...
                                                    <TableCell>
                                                        <div className="text-sm">
                                                            <div className="font-medium text-gray-900">
                                                                {monitor.stats?.uptimePercentage?.toFixed(2) || "N/A"}%
                                                            </div>
                                                            <div className="text-gray-500">
                                                                {monitor.stats?.avgResponseTime || "N/A"}ms avg
                                                            </div>
                                                        </div>
                                                    </TableCell>
                                                    <TableCell>
                                                        <div className="flex items-center space-x-2">
                                                            ...
                                                            <DropdownMenu>
                                                                ...
                                                                <DropdownMenuContent align="end">
                                                                    ...
                                                                    <DropdownMenuItem
                                                                        onClick={() => handleDeleteMonitor(monitor.id, monitor.url)}
